...
def run_match(job_id):
    ...
    for cv_row in cvs:
        ...
        ats_result = nlp_engine().calculate_ats_score(
            cv_text=cv.get("original_text", ""), job_description=job["description"], pdf_warnings=pdf_warnings
        )
...
def delete_cv(cv_id):
    try:
        deleted, cleanup_pending = delete_cv_transaction(get_db(), cv_id, current_app.config["UPLOAD_FOLDER"])
    ...
    if cleanup_pending:
        current_app.logger.error("CV database records deleted but staged file cleanup failed")
        return jsonify(
            {
                "success": True,
