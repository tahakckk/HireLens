...
def job_search_generate_cv():
    ...
    return jsonify({"success": True, "session_id": session_id, "cv_data": optimized_cv_data})
...
def generate_docx_buffer(cv_data):
    ...
    if experiences:
        ...
        for exp in experiences:
            ...
            meta_text = []
            if exp.get("location"):
                meta_text.append(exp["location"])
            if exp.get("period"):
                meta_text.append(exp["period"])
    ...
    if cv_data.get("languages"):
        ...
        langs = [f"{language.get('language', '')} ({language.get('level', '')})" for language in cv_data["languages"]]
        p = doc.add_paragraph()
...
def job_search_download_cv(session_id):
    ...
    doc = SimpleDocTemplate(
        buffer, pagesize=A4, leftMargin=20 * mm, rightMargin=20 * mm, topMargin=15 * mm, bottomMargin=15 * mm
    )
    ...
    styles.add(
        ParagraphStyle(name="CVSubtitle", fontSize=11, fontName=font_bold, spaceAfter=2, textColor=HexColor("#000000"))
    )
    styles.add(
        ParagraphStyle(name="CVMeta", fontSize=9, fontName=font_italic, spaceAfter=3, textColor=HexColor("#444444"))
    )
    styles.add(
        ParagraphStyle(
            name="CVBody",
    ...
    if languages:
        ...
        elements.append(HRFlowable(width="100%", thickness=0.5, color=HexColor("#d0d0e8"), spaceAfter=6))
        lang_text = ", ".join([f"{language.get('language', '')} ({language.get('level', '')})" for language in languages])
