...
def job_search_upload_cv():
    ...
    try:
        ...
        profile_data = current_app.extensions["extractive_cv_generator"].parse_cv(raw_text)
...
def job_search_parse_job():
    ...
    if result['success'] and result.get('description'):
        extracted_skills = current_app.extensions["nlp_engine"].extract_skills(result['description'])
